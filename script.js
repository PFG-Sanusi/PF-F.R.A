// Common JavaScript functionality for all pages - Optimized for Performance
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize common functionality with error handling
    try {
        ...
        if ('performance' in window) {
            console.log('DOM Content Loaded in:', performance.now(), 'ms');
        }
    } catch (error) {
...
function initializeFormValidation(form) {
    ...
    inputs.forEach(input => {
        ...
        if (input.tagName === 'TEXTAREA' && input.hasAttribute('maxlength')) {
            initializeCharacterCounter(input);
        }
...
function initializeAnimations() {
    ...
    // Observe elements for animation with throttling
    const elementsToObserve = document.querySelectorAll('.service-card, .feature-item, .process-step, .faq-item');
