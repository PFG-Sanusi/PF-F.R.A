...
function showMapLoading(message = 'Loading...') {
...
function showMapError(errorMessage, errorType = 'network') {
...
function updateNetworkStatus(status) {
    ...
    if (statusElement) {
        ...
        // Hide after 3 seconds for online status
        if (status === 'online') {
            setTimeout(() => {
...
function handleMapRetry() {
    ...
    if (retryCount <= maxRetries) {
        showMapLoading(`Retrying... (${retryCount}/${maxRetries})`);
        setTimeout(() => {
            initializeMapWithErrorHandling();
        }, 1000 * retryCount); // Exponential backoff
    } else {
        showMapError('Unable to load map after multiple attempts. Please check your internet connection and try again later.', 'max_retries');
    }
}
...
function enableOfflineMode() {
    ...
    // Disable map functionality that requires internet
    const mapControls = document.querySelectorAll('.map-controls button');
    mapControls.forEach(button => {
        if (button.id !== 'drawPolygon' && button.id !== 'clearMap') {
            button.disabled = true;
            button.title = 'Offline mode - Limited functionality';
        }
    });
    
    // Show offline message
    showMapLoading('Offline Mode - Limited functionality available');
    setTimeout(() => {
        hideMapLoading();
    }, 2000);
}
...
function validateFile(file) {
    ...
    const allowedTypes = ['.kml', '.geojson', '.json', '.gpx', '.shp', '.zip'];
    ...
    if (!allowedTypes.includes(fileExtension)) {
        return {
            valid: false,
            error: `File "${file.name}" has an unsupported format. Supported formats: KML, GeoJSON, Shapefile, GPX.`
        };
    }
    ...
    if (file.type && !allowedMimeTypes.includes(file.type)) {
        return {
            valid: false,
            error: `File "${file.name}" has an unsupported MIME type.`
        };
    }
...
function handleFileUpload(files) {
    ...
    // Show errors if any
    if (errors.length > 0) {
        showUploadStatus(errors.join(' '), 'error');
        return;
    }
    
    // Process valid files
    if (validFiles.length > 0) {
        processFileUpload(validFiles);
    }
}

function processFileUpload(files) {
    showUploadProgress(0);
    showUploadStatus(`Processing ${files.length} file(s)...`, 'info');
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    if (processedCount === files.length) {
                        ...
                        // Here you would actually process the file content
                        console.log('File processed:', file.name);
                    }
                }, 1000);
            } catch (error) {
                showUploadStatus(`Error processing file "${file.name}": ${error.message}`, 'error');
            }
        };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    checkNetworkConnectivity().then(isOnline => {
        if (!isOnline) {
            ...
            showMapError('No internet connection detected. Some features may not work properly.', 'network');
        } else {
...
function initializeMapWithErrorHandling() {
    try {
        ...
        showMapLoading('Initializing map...');
        ...
        map.on('tileerror', function(e) {
            ...
            // Don't show error for individual tile failures, just log them
        });
        ...
        // Set up layers with error handling
        setupMapLayersWithFallbacks();
        ...
        // Set up event listeners for error handling buttons
        setupErrorHandlingEventListeners();
        
        console.log('Map initialized successfully');
        
    } catch (error) {
        console.error('Map initialization error:', error);
        showMapError(`Failed to initialize map: ${error.message}`, 'initialization');
    }
}

function setupMapLayersWithFallbacks() {
    const tileOptions = {
        attribution: '',
        maxZoom: 18,
        minZoom: 1,
    ...
    try {
        ...
        osmLayer = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            ...
            updateWhenZooming: false,
            updateWhenIdle: true,
            errorTileUrl: tileOptions.errorTileUrl
        });
...
function setupErrorHandlingEventListeners() {
    // Retry button
    const retryButton = document.getElementById('retryMap');
    if (retryButton) {
        retryButton.addEventListener('click', handleMapRetry);
    }
    
    // Offline mode button
    const offlineButton = document.getElementById('offlineMode');
    if (offlineButton) {
        offlineButton.addEventListener('click', enableOfflineMode);
    }
    ...
    window.addEventListener('offline', () => {
        updateNetworkStatus('offline');
        if (!isOfflineMode) {
            showMapError('Connection lost. Some features may not work properly.', 'network');
        }
    });
}
